...
module.exports = function ({state, dispatch, next}) {
  ...
  function button (button) {
    return html`<div>
      <button class="border-radius border border-black bold width-full padding-2 background-white black hover-background-black hover-white" type="button" onclick=${() => dispatch('set', button.time, button.message)}>${button.title}</button>
    </div>`
  }
}
